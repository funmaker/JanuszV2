...
extend(RateTransposer.prototype, {
  ...
  process: function (numFrames) {
    // TODO aa filter
    numFrames = Math.min(numFrames, this._inputBuffer.frameCount) || this._inputBuffer.frameCount;
    this._outputBuffer.ensureAdditionalCapacity(numFrames / this._rate + 1);
    var numFramesOutput = this._transpose(numFrames);
    this._inputBuffer.receive(numFrames);
...
FifoSampleBuffer.prototype = {
  ...
  ensureCapacity: function (numFrames) {
    ...
    if (this._vector.length < minLength) {
      var newVector = new Int16Array(minLength);
...
extend(SimpleFilter.prototype, {
  ...
  fillInputBuffer: function(numFrames) {
    var samples = new Int16Array(numFrames * 2);
...
function Stretch(createBuffers) {
  ...
  this._tempo = 1;
  this.setParameters(SAMPLE_RATE, BUFFER_SIZE / SAMPLE_RATE * 1000, DEFAULT_SEEKWINDOW_MS, DEFAULT_OVERLAP_MS);
}
...
extend(Stretch.prototype, {
  ...
  calculateOverlapLength: function (overlapInMsec) {
    ...
    this.pRefMidBuffer = new Int16Array(this.overlapLength * 2);
    this.pMidBuffer = new Int16Array(this.overlapLength * 2);
  },
  ...
  process: function(numFrames = this._inputBuffer.frameCount) {
    var ovlSkip;
    var offset;
    var temp;
    var i;
    ...
    while (this._inputBuffer.frameCount >= this.sampleReq && processed < numFrames) {
      // If tempo differs from the normal ('SCALE'), scan for the best overlapping
      // position
      offset = this.seekBestOverlapPosition();
      processed += this.sampleReq;
...
extend(SoundTouch.prototype, {
  ...
  process: function (numFrames) {
    numFrames = Math.min(this._inputBuffer.frameCount, numFrames) || this._inputBuffer.frameCount;
    let processed = 0;
    if (this._rate > 1.0) {
      processed += this.tdStretch.process(numFrames);
      processed += this.rateTransposer.process();
    }
    else {
      processed += this.rateTransposer.process(numFrames);
      processed += this.tdStretch.process();
    }
    return processed;
  }
});
