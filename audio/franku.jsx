...
setInterval(() => {
	if(counter > 0) {
		counter--;
		for(let callback of triggerCallbacks) {
			callback(counter);
		}
	}
}, 1000);
...
export default class Franku extends React.Component {
	...
	draw() {
		...
		for(let n = 0; n < width * height * 4; n += 4) {
			let [r, g, b] = [frame.data[n], frame.data[n + 1], frame.data[n + 2]];
			let [targetRed, targetGreen, targetBlue] = [44, 255, 26];
			let [diffRed, diffGreen, diffBlue] = [Math.abs(r - targetRed), Math.abs(g - targetGreen), Math.abs(b - targetBlue)];
			let diff = diffRed + diffGreen + diffBlue;
