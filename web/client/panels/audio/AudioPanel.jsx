...
export class Panel extends React.Component {
  ...
  handleMessage = msg => {
    ...
    switch(msg.type) {
      ...
      case packets.types.DEVICES_UPDATE: {
        this.setState(state => {
          const devices = merge(state.devices, msg.devices);
          ...
          if(Object.values(devices).some(device => device.outputActivity)) {
            ...
            for(const con of Object.values(state.connections)) {
      ...
      case packets.types.CONNECTIONS_UPDATE: {
        this.setState(state => {
          ...
          for(const uuid of Object.keys(msg.connections)) {
  ...
  onMouseMove = ev => {
    ...
    this.setState(state => ({
      posx: state.posx + ev.nativeEvent.movementX,
      posy: state.posy + ev.nativeEvent.movementY,
    }));
  };
  ...
  onTouchMove = ev => {
    ...
    this.setState(state => ({
      posx: state.posx + ev.touches[0].clientX - state.touchX,
      posy: state.posy + ev.touches[0].clientY - state.touchY,
      touchX: ev.touches[0].clientX,
      touchY: ev.touches[0].clientY,
    }));
  };
